...
function AnimatedUserCard({ item, index, onPress }: { item: JPUser; index: number; onPress: () => void }) {
  ...
  const onPressIn = () => Animated.spring(scaleAnim, { toValue: 0.96, useNativeDriver: true, speed: 50 }).start();
...
export default function PeopleScreen() {
  ...
  const headerHeight = scrollY.interpolate({ inputRange: [0, 80], outputRange: [56, 36], extrapolate: 'clamp' });
  ...
  return (
    <View style={styles.container}>
      <Animated.View style={[styles.cacheBar, { height: headerHeight, opacity: headerOpacity }]}>
        <Ionicons name="time-outline" size={14} color="#FF2D55" />
        <Text style={styles.cacheText}>
          jsonplaceholder.typicode.com{cacheTime ? ` • ${cacheTime}` : ''}
        </Text>
          ...
          <AnimatedUserCard
            ...
            onPress={() => router.push({
              ...
              params: {
                ...
                color: AVATAR_COLORS[(item.id - 1) % AVATAR_COLORS.length],
...
const styles = StyleSheet.create({
  ...
  cacheBar: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFF0F3', paddingHorizontal: 16, gap: 6, overflow: 'hidden' },
